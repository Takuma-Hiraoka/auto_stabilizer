...
public:
  ...
  std::vector<std::vector<cnoid::Vector3> > legHull = std::vector<std::vector<cnoid::Vector3> >(2, std::vector<cnoid::Vector3>{cnoid::Vector3(0.095,0.050,0.0),cnoid::Vector3(-0.095,0.050,0.0),cnoid::Vector3(-0.095,-0.050,0.0),cnoid::Vector3(0.095,-0.050,0.0)}); // 要素数2. rleg: 0. lleg: 1. endeffector frame.  凸形状で,上から見て半時計回り. Z成分はあったほうが計算上扱いやすいからありにしているが、0でなければならない. 単位[m]. JAXONでは、COPがY -0.1近くにくるとギア飛びしやすいので、少しYの下限を少なくしている
...
public:
  ...
  std::vector<bool> prevOriginLeg = std::vector<bool>{true, false}; // 要素数2.  rleg: 0. lleg: 1. 一つ前の周期でweightが1であったかどうか
